...
def synced(sender: Sender, receiver: Receiver, previous_last_sync_id: int) -> bool:
    return (
        ...
        and sender._last_sync_id == receiver._last_sync_id
...
async def test_harvester_restart(environment: Environment) -> None:
    ...
    # Sleep one second to get a different `sync_id` on the harvester
...
async def test_farmer_restart(environment: Environment) -> None:
    ...
    assert len(env.farmer.plot_sync_receivers) == 2
    # Reset the expected data for both harvesters and re-add all directories because of the restart
    for i in [0, 1]:
        env.expected[i].reset()
        env.add_all_directories(i)
    # Run the refresh two times and make sure everything recovers and stays recovered after farmer restart
    await env.run_sync_test()
    await env.run_sync_test()
