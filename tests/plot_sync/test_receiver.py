...
async def run_sync_step(receiver: Receiver, sync_step: SyncStepData, expected_state: State) -> None:
    ...
    last_activity_before = receiver._last_activity
    ...
    assert receiver.state() != expected_state
    assert receiver._last_activity != last_activity_before
...
async def test_reset() -> None:
    ...
    # But _last_activity should be bumped
    assert receiver._last_activity != last_activity_before
...
async def test_to_dict() -> None:
    ...
    assert "last_activity" in plot_sync_dict_1 and plot_sync_dict_1["last_activity"] == receiver._last_activity
